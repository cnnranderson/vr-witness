...
inline std::string sha256(const std::filesystem::path& path) {
    ...
    if (!file)
        throw std::runtime_error("Cannot read executable for hash verification");
    std::vector<unsigned char> bytes((std::istreambuf_iterator<char>(file)), {});
    if (bytes.size() != 6864896)
        throw std::runtime_error("Unsupported executable size");
...
inline void match(std::uintptr_t base, std::uintptr_t rva, const char* hex) {
    ...
    SIZE_T received{};
    if (!ReadProcessMemory(GetCurrentProcess(), reinterpret_cast<void*>(base + rva), actual.data(),
