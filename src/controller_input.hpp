...
inline constexpr unsigned role_slot = 17, connected_slot = 20, property_slot = 23, state_slot = 32,
                          focus_slot = 39;
...
inline void select_axis(Hand& hand, bool legacy) {
    ...
    if (sticks == 0 && legacy && hand.types[0] == 1 && hand.types[1] == 3 && hand.types[2] == 3 &&
        hand.types[3] == -1 && hand.types[4] == -1) {
        hand.stick = 0;
        hand.legacy_axis = true;
    }
}
...
public:
    ...
    Axis update(const Hand& hand, bool allowed) {
        if (!allowed || !hand.valid || hand.stick < 0 || hand.stick >= 5 ||
...
inline bool blend(Vec3& native, Axis stick, const Context& c) {
    if (!finite(native) || !finite(c.forward) || !finite(c.left) || !finite(stick))
        return false;
    const float f = length(c.forward), l = length(c.left);
    if (f < .9f || f > 1.1f || l < .9f || l > 1.1f)
        return false;
    // Native +strafe is the A/left direction in this verified build.
    Vec3 add{c.forward.x * stick.y - c.left.x * stick.x, c.forward.y * stick.y - c.left.y * stick.x,
    ...
    if (!std::isfinite(n) || !std::isfinite(cap))
        return false;
    if (n > cap) {
        result.x *= cap / n;
        result.y *= cap / n;
