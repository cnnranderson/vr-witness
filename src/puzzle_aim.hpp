...
inline Vec3 multiply(Vec3 a,float s){return {a.x*s,a.y*s,a.z*s};}
inline float dot(Vec3 a,Vec3 b){return a.x*b.x+a.y*b.y+a.z*b.z;}
inline Vec3 cross(Vec3 a,Vec3 b){return {a.y*b.z-a.z*b.y,a.z*b.x-a.x*b.z,a.x*b.y-a.y*b.x};}
inline bool valid_pose(const TrackedPose& p) {
    if(!p.valid || !p.connected || p.tracking_result!=200) return false;
    for(const auto& row:p.matrix)for(float f:row)if(!std::isfinite(f))return false;
    Vec3 cols[3];
    for(int i=0;i<3;++i)cols[i]={p.matrix[0][i],p.matrix[1][i],p.matrix[2][i]};
    for(auto c:cols)if(std::abs(dot(c,c)-1.f)>.03f)return false;
...
public:
    ...
    void reset() {rotation_={};w_=1.f;device_=invalid_device;button_armed_=false;}
    // Return true only when a fresh, non-drawing button press changes calibration.
    bool update(std::uint32_t device,bool allowed,bool button,bool drawing,
                const TrackedPose& head,const TrackedPose& hand) {
        if(device!=device_) {reset();device_=device;}
        if(!allowed || device>=16 || !valid_pose(head) || !valid_pose(hand)) {button_armed_=false;return false;}
        if(!button) {button_armed_=true;return false;}
        if(!button_armed_)return false;
        button_armed_=false;
        ...
        if(w<.01f || !std::isfinite(norm) || norm<.01f)return false;
        rotation_=multiply(xyz,1.f/norm);w_=w/norm;
        return true;
    }
...
inline bool aim_target(const TrackedPose& head,const TrackedPose& hand,const AimFrame& f,Axis& target,const AimCalibration& calibration = AimCalibration{}) {
    if(!valid_pose(head)||!valid_pose(hand)||!f.valid || !finite(f.origin)||!finite(f.plane)||!finite(f.u)||!finite(f.v) ||
       !std::isfinite(f.scale)||f.scale<.001f||f.scale>10.f ||
       !std::isfinite(f.bounds.x)||!std::isfinite(f.bounds.y)||f.bounds.x<=0||f.bounds.y<=0||f.bounds.x>4||f.bounds.y>4)return false;
    if(std::abs(dot(f.u,f.u)-1.f)>.03f||std::abs(dot(f.v,f.v)-1.f)>.03f||std::abs(dot(f.u,f.v))>.02f)return false;
    ...
    const Vec3 direction=add(add(multiply(f.u,relative.x),multiply(f.v,relative.y)),multiply(forward,-relative.z));
    const float denominator=dot(direction,forward);
    if(denominator<.15f)return false;
    ...
    return std::isfinite(target.x)&&std::isfinite(target.y)&&target.x>=0&&target.y>=0&&target.x<=f.bounds.x&&target.y<=f.bounds.y;
}
...
class Pointing {
    ...
    std::uint64_t tick_{};
public:
    void reset(){device_=invalid_device;armed_=false;tick_=0;}
    // Return whether delta should replace native input; tick is monotonic milliseconds.
    bool update(std::uint32_t device,bool allowed,bool trigger,const Axis& target,const Axis& current,Axis& delta,
                std::uint64_t tick,float view_width,const AimSettings& settings) {
        if(!allowed || device>=16 || !valid_settings(settings) || !std::isfinite(view_width) || view_width<=0 || view_width>4 ||
           !std::isfinite(current.x)||!std::isfinite(current.y)||!std::isfinite(target.x)||!std::isfinite(target.y)) {reset();return false;}
        if(device_!=device){reset();device_=device;}
        // No cursor jump on enabling/reacquiring while the draw trigger is held.
        if(!armed_){armed_=!trigger;tick_=tick;return false;}
        if(tick<=tick_) {delta={};return true;}
        const float dt=static_cast<float>(tick-tick_)/1000.f;tick_=tick;
        if(dt>.1f) {reset();return false;}
        delta={target.x-current.x,target.y-current.y};
        const float n=std::hypot(delta.x,delta.y);
        if(!std::isfinite(n)){reset();return false;}
        if(n<.0004f)delta={};
        else {
            const float alpha=settings.smoothing_ms ? -std::expm1(-dt*1000.f/settings.smoothing_ms) : 1.f;
