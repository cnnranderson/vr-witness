...
Options options(int argc, wchar_t** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (name == L"--controller-snap") {
            result.controller_snap = true;
            continue;
        }
        if (name == L"--controller-snap-trace") {
            result.controller_snap_trace = true;
            continue;
        }
        if (name == L"--controller-aim") {
            result.controller_aim = true;
            continue;
        }
        if (name == L"--controller-aim-trace") {
            result.controller_aim_trace = true;
            continue;
        }
        if (name == L"--controller-move") {
            result.controller_move = true;
            continue;
        }
        if (name == L"--test-host") {
        ...
        if (name == L"--render-trace") {
            result.render_trace = true;
            continue;
        }
        if (name == L"--defer-cursor") {
            result.defer_cursor = true;
            continue;
        }
        if (name == L"--defer-menu") {
            result.defer_menu = true;
            continue;
        }
        if (name == L"--redraw-pause") {
            result.redraw_pause = true;
            continue;
        }
        if (i + 1 >= argc)
        ...
        if (name == L"--pid")
        ...
        else if (name == L"--game-exe")
            result.expected_exe = value;
        else if (name == L"--log")
            result.log = value;
        else if (name == L"--aim-speed-percent") {
            result.aim_speed_percent = number(value);
            ...
            if (value == L"22.5")
                ...
                throw std::runtime_error("Snap angle must be 22.5, 45 or 90 degrees");
            ...
            if (!result.session.empty())
                throw std::runtime_error("Choose one session control");
            result.session = value;
            result.controller_session = name == L"--controller-session";
        } else
            throw std::runtime_error("Unknown option");
    }
    if (result.controller_snap && !((result.controller_trace && result.session.empty()) ||
                                    (result.controller_session && result.session == L"start")))
        throw std::runtime_error("Snap turning requires a controller capture or session start");
    ...
    if (result.aim_speed_percent < 10 || result.aim_speed_percent > 300 || result.aim_smoothing_ms > 250)
        throw std::runtime_error(
            "Aim speed must be 10..300 percent of view width/second; smoothing 0..250 ms");
    if (result.aim_settings_given &&
        !(result.controller_session && result.session == L"start" && result.controller_aim))
        throw std::runtime_error("Aim settings require controller session start with --controller-aim");
    if (result.controller_aim && !((result.controller_trace && result.session.empty()) ||
                                   (result.controller_session && result.session == L"start")))
        throw std::runtime_error("Aiming requires a controller capture or session start");
    if (result.controller_aim_trace && (!result.controller_trace || !result.session.empty()))
        throw std::runtime_error("Passive aim trace requires a bounded controller capture");
    if (result.controller_aim && result.controller_aim_trace)
        throw std::runtime_error("Choose aiming or passive aim trace");
    if (result.controller_move && !result.controller_trace)
        throw std::runtime_error("Movement flag requires --controller-trace");
    if (result.controller_legacy_axis0 && !result.controller_trace && !result.controller_session)
        throw std::runtime_error("Legacy axis requires controller capture/session");
    if (result.controller_trace && (result.render_trace || result.defer_cursor || result.defer_menu ||
                                    result.redraw_pause || !result.session.empty()))
        throw std::runtime_error("Choose timed capture or session control");
    if (!result.session.empty()) {
        if (result.render_trace || result.defer_cursor || result.defer_menu || result.redraw_pause)
            throw std::runtime_error("Choose session or timed render trace");
        if (result.session != L"start" && result.session != L"stop" && result.session != L"enable" &&
            result.session != L"disable" && result.session != L"status")
            throw std::runtime_error("Unknown session command");
        if (result.controller_session)
            result.controller_trace = true;
        else
            result.render_trace = true;
    ...
    if (result.test_host) {
        if (!result.expected_exe.empty())
            throw std::runtime_error("--test-host cannot be combined with --game-exe");
        result.expected_exe = std::filesystem::path(module_path()).parent_path() /
                              (result.controller_trace ? L"witness-input-test-host.exe"
                               : result.render_trace   ? L"witness-render-test-host.exe"
                                                       : L"witness-probe-test-host.exe");
    } else if (result.expected_exe.empty() ||
               _wcsicmp(result.expected_exe.filename().c_str(), L"witness64_d3d11.exe")) {
        throw std::runtime_error(
            "Only witness64_d3d11.exe is supported; provide its full path with --game-exe");
    }
    ...
    if (static_cast<int>(result.defer_cursor) + static_cast<int>(result.defer_menu) +
            static_cast<int>(result.redraw_pause) >
        1)
...
public:
    explicit LocalLibrary(const std::filesystem::path& path) {
        ...
        if (!value)
            throw win_error("LoadLibraryExW (local probe)");
    }
...
public:
    RemoteBuffer(HANDLE process, const void* data, std::size_t size) : process_(process) {
        ...
        if (!value)
            throw win_error("VirtualAllocEx");
        SIZE_T written = 0;
    ...
    ~RemoteBuffer() {
        if (value)
            VirtualFreeEx(process_, value, 0, MEM_RELEASE);
    }
...
DWORD remote_call(HANDLE process, std::uintptr_t function, void* parameter, DWORD timeout,
                  RemoteBuffer* allocation = nullptr) {
    Handle thread(CreateRemoteThread(process, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(function),
                                     parameter, 0, nullptr));
    if (!thread)
        throw win_error("CreateRemoteThread");
    ...
    if (wait != WAIT_OBJECT_0) {
        ...
        if (allocation)
            allocation->preserve();
        throw std::runtime_error(
            "Remote thread did not finish. Probe may remain loaded; close and restart the target before retrying.");
    }
    DWORD result = 0;
    if (!GetExitCodeThread(thread.get(), &result))
        throw win_error("GetExitCodeThread");
    return result;
}
...
std::uintptr_t remote_system_function(DWORD pid, const char* name) {
    ...
    if (!function)
        throw win_error("GetProcAddress (system function)");
    ...
    for (const auto& module : modules(pid)) {
        if (same_path(module.path, path)) {
            if (offset >= module.size)
                throw std::runtime_error("System function lies outside remote image");
...
int session_control(const Options& config, HANDLE process, const std::filesystem::path& dll) {
    ...
    request.command = config.session == L"start"     ? SessionCommand::start
                      ...
                      : config.session == L"disable" ? SessionCommand::disable
    ...
    if (request.command == SessionCommand::start && !config.no_log) {
        ...
        if (std::filesystem::exists(config.log))
            throw std::runtime_error("Refusing to overwrite existing log");
    ...
    if (!run)
        throw win_error("GetProcAddress (WitnessSessionControl)");
    ...
    if (!base) {
        ...
        remote_call(process, remote_system_function(config.pid, "LoadLibraryW"), buffer.value, 15000,
                    &buffer);
        base = find_module(config.pid, dll);
        if (!base)
            throw std::runtime_error("Render DLL did not load");
    }
    ...
    SIZE_T received{};
    if (!ReadProcessMemory(process, buffer.value, &request, sizeof(request), &received) ||
    ...
    const char* state = request.state == SessionState::running    ? "running"
                        : request.state == SessionState::stopping ? "stopping"
    ...
    if (result)
        throw std::runtime_error(
            "Session command failed (active diagnostic/session, fault, or log/setup failure); inspect session log and status");
    return 0;
}
...
int input_session_control(const Options& config, HANDLE process, const std::filesystem::path& dll) {
    ...
    if (!run)
        throw win_error("GetProcAddress (WitnessInputSessionControl)");
    const auto offset = reinterpret_cast<std::uintptr_t>(run) - reinterpret_cast<std::uintptr_t>(local.value);
    if (!base) {
        const auto path = dll.wstring();
        RemoteBuffer buffer(process, path.c_str(), (path.size() + 1) * sizeof(wchar_t));
        ...
        base = find_module(config.pid, dll);
        if (!base)
            throw std::runtime_error("Input DLL did not load");
    }
    RemoteBuffer buffer(process, &request, sizeof(request));
    const auto result = remote_call(process, base + offset, buffer.value, 15000, &buffer);
    SIZE_T received{};
    if (!ReadProcessMemory(process, buffer.value, &request, sizeof(request), &received) ||
        received != sizeof(request))
        throw win_error("Read input session status");
    const char* state = request.state == SessionState::running    ? "running"
                        : request.state == SessionState::stopping ? "stopping"
                        : request.state == SessionState::failed   ? "failed"
                                                                  : "stopped";
    std::cout << "{\"state\":\"" << state << "\",\"enabled\":" << (request.enabled ? "true" : "false")
              ...
              << ",\"pointing\":" << (request.pointing ? "true" : "false")
              << ",\"aim_enabled\":" << ((request.enabled && request.pointing) ? "true" : "false")
              << ",\"stick_cursor_speed_percent\":" << request.stick_speed_percent
              << ",\"aim_speed_percent\":" << request.aim_speed_percent
              << ",\"aim_smoothing_ms\":" << request.aim_smoothing_ms
              << ",\"puzzle_back_presses\":" << request.puzzle_back_presses
              << ",\"stick_applied\":" << request.stick_applied
...
int wmain(int argc, wchar_t** argv) {
    if (argc == 1 || (argc == 2 && std::wstring(argv[1]) == L"--help")) {
        std::wcout
            << L"Witness VR loader and cursor session control\n"
    ...
    try {
        ...
        const auto dll = std::filesystem::path(module_path()).parent_path() /
                         (config.controller_trace ? L"witness-input-probe.dll"
                          : config.render_trace   ? L"witness-render-probe.dll"
                                                  : L"witness-probe.dll");
        ...
        if (!process)
            throw win_error("OpenProcess");
        if (!same_path(process_path(process.get()), config.expected_exe))
            throw std::runtime_error(
                "Target process path does not match the expected executable; nothing injected");
        ...
        if (config.controller_trace) {
            for (const auto& m : modules(config.pid)) {
                if (!_wcsicmp(m.name.c_str(), L"witness-input-probe.dll") && !same_path(m.path, dll))
                    throw std::runtime_error(
                        "A different input DLL build remains loaded; close and restart the game before changing builds");
            }
        }
        if (find_module(config.pid, dll) && !config.render_trace && !config.controller_trace)
            throw std::runtime_error(
                "Probe is already loaded. Wait for the other probe, or restart the game before retrying");
        ...
        if (!config.session.empty())
            return config.controller_session ? input_session_control(config, process.get(), dll)
                                             : session_control(config, process.get(), dll);

        const auto log_path = config.log.wstring();
        if (log_path.size() >= kPathCapacity)
            throw std::runtime_error("Log path is too long");
        if (std::filesystem::exists(config.log))
            throw std::runtime_error("Refusing to overwrite an existing log; choose a new filename");
        std::filesystem::create_directories(config.log.parent_path());
        ...
        request.reserved =
            config.controller_trace
                ? ((config.controller_move ? 1 : 0) | (config.controller_legacy_axis0 ? 2 : 0) |
                   (config.controller_aim ? 4 : 0) | (config.controller_aim_trace ? 8 : 0) |
                   (config.controller_snap ? (16 | (config.snap_steps == 1   ? 64
                                                    : config.snap_steps == 4 ? 128
                                                                             : 0))
                                           : 0) |
                   (config.controller_snap_trace ? 32 : 0))
            : config.redraw_pause ? 3
            : config.defer_menu   ? 2
            : config.defer_cursor ? 1
                                  : 0;
        ...
        if (!run)
            throw win_error("GetProcAddress (WitnessProbeRun)");
        const auto offset =
            reinterpret_cast<std::uintptr_t>(run) - reinterpret_cast<std::uintptr_t>(local.value);
        ...
        if (!remote_base)
            throw std::runtime_error("DLL did not load in target (check matching architecture/dependencies)");
        ...
        if (result > static_cast<DWORD>(ProbeResult::module_snapshot_failed))
            throw std::runtime_error(
                "Probe thread terminated unexpectedly; restart the target before retrying");
        if (!config.render_trace && !config.controller_trace) {
            const auto unloaded =
                remote_call(process.get(), unload, reinterpret_cast<void*>(remote_base), 15000);
        ...
        std::wcout
            << ((config.render_trace || config.controller_trace)
                    ? L"Capture finished; see log for hook/session status. DLL retained until game exit. Log: "
                    : L"Probe finished and unloaded. Log: ")
            << config.log.wstring() << L"\n";
        return 0;
