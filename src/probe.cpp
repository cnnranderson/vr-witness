...
std::string quote(const std::string& value) {
    ...
    for (const unsigned char c : value) {
        ...
        case '\t': out << "\\t"; break;
        default:
            if (c < 0x20) out << "\\u" << std::hex << std::setw(4) << std::setfill('0') << static_cast<unsigned>(c);
...
bool interesting(const std::wstring& name) {
    const wchar_t* names[] = {L"d3d11.dll", L"dxgi.dll", L"openvr_api.dll", L"vrclient_x64.dll",
        L"LibOVRRT64_1.dll", L"openxr_loader.dll"};
    for (const auto* expected : names) if (_wcsicmp(name.c_str(), expected) == 0) return true;
    return false;
}
} // namespace
...
extern "C" __declspec(dllexport) DWORD WINAPI WitnessProbeRun(void* argument) noexcept {
    ...
    try {
        ...
        if (!path.is_absolute()) return static_cast<DWORD>(ProbeResult::invalid_request);
        std::ofstream out(path, std::ios::binary | std::ios::trunc);
        if (!out) return static_cast<DWORD>(ProbeResult::cannot_open_log);
        event(out, "probe.started", ",\"protocol\":1,\"mode\":\"observe_only\",\"game_exe\":" +
            quote(utf8(module_path())) + ",\"duration_ms\":" + std::to_string(request.duration_ms));
        event(out, "probe.capabilities", ",\"camera_writes\":false,\"render_hooks\":false,\"vr_initialized_by_probe\":false");
        ...
        do {
            ...
            for (const auto& module : loaded) {
                ...
                if (old != seen.end() && old->second == module.path) continue;
                event(out, "module.loaded", ",\"name\":" + quote(utf8(module.name)) +
                    ",\"path\":" + quote(utf8(module.path)) + ",\"base\":" + quote(hex(module.base)) +
            ...
            for (const auto& [base, name] : seen) {
                ...
                if (now == current.end() || now->second != name)
                    event(out, "module.unloaded", ",\"path\":" + quote(utf8(name)) + ",\"base\":" + quote(hex(base)));
            }
            if (first) {
                // A loaded module does not prove its VR implementation was entered.
                for (const auto* name : {L"d3d11.dll", L"dxgi.dll", L"openvr_api.dll", L"vrclient_x64.dll", L"LibOVRRT64_1.dll", L"openxr_loader.dll"}) {
                    const bool present = std::any_of(loaded.begin(), loaded.end(), [name](const Module& module) {
                        return _wcsicmp(module.name.c_str(), name) == 0;
                    });
                    event(out, "runtime.snapshot", ",\"module\":" + quote(utf8(name)) + ",\"loaded\":" + (present ? "true" : "false"));
                }
                first = false;
            }
            seen = std::move(current);
            const ULONGLONG elapsed = GetTickCount64() - start;
            if (elapsed >= request.duration_ms) break;
