...
class PuzzleBack {
    ...
    bool armed_{};
public:
    void reset(){device_=invalid_device;armed_=false;}
    ...
    bool update(const Hand& hand,bool allowed) {
        if(!allowed || !hand.valid || hand.device>=16){reset();return false;}
        if(device_!=hand.device){reset();device_=hand.device;}
        // Left-B-only capture: bit 1 on the current Knuckles legacy binding.
        if(!(hand.state.pressed&(1ull<<1))){armed_=true;return false;}
