...
inline std::wstring module_path(HMODULE module = nullptr) {
    ...
    if (!count || count >= result.size()) throw win_error("GetModuleFileNameW");
    result.resize(count);
    return result;
}
...
inline std::vector<Module> modules(DWORD pid) {
    ...
    Handle snapshot(raw);
    if (!snapshot) throw win_error("CreateToolhelp32Snapshot");
    MODULEENTRY32W entry{};
    entry.dwSize = sizeof(entry);
    if (!Module32FirstW(snapshot.get(), &entry)) throw win_error("Module32FirstW");
    ...
    if (GetLastError() != ERROR_NO_MORE_FILES) throw win_error("Module32NextW");
    return result;
}
...
inline std::string utf8(const std::wstring& value) {
    ...
    if (!count) throw win_error("WideCharToMultiByte");
