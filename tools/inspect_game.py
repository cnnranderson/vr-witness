...
class PE:
    def __init__(self, path: Path):
        ...
        for i in range(count):
            ...
            self.sections.append(dict(name=name, rva=rva, virtual_size=vsize, file_offset=offset, file_size=rawsize))
    ...
    def imports(self):
        ...
        for descriptor in range(min(size // 20, 4096)):
            ...
            for index in range(65536):
                thunk = self.unpack("<Q" if self.is64 else "<I", self.offset((original or first) + index * width))[0]
                if not thunk:
                    break
    ...
    def markers(self):
        wanted = re.compile(r"^(?:IVR\w+_\d+|VR_\w+|ovr_\w+|updateCamera|player_camera_\w+|vr_\w+|teleport_in_vr_enabled|free_camera_speed|stereo_base|OVR:.*)$")
        result = []
        for match in re.finditer(rb"[\x20-\x7e]{4,}\x00", self.data):
            ...
            if wanted.fullmatch(value):
                ...
                result.append(dict(text=value, file_offset=f"0x{match.start():x}", rva=None if rva is None else f"0x{rva:x}"))
        return result
    ...
    def report(self):
        return dict(
            ...
            size_bytes=len(self.data), machine=f"0x{self.machine:04x}",
            image_base=f"0x{self.image_base:x}", image_size=self.image_size,
            pe_timestamp=self.timestamp, sections=self.sections,
            imports=self.imports(), markers=self.markers(),
        )
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parents[1] / "out/reports/game-inspection.json")
    ...
    report = dict(generated_utc=datetime.now(timezone.utc).isoformat(), game_directory=str(root), images=images,
