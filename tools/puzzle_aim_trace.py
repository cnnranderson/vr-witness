...
def sample(reader, base):
    ...
    state.update(
        ...
        camera_angles_by_rva={
            hex(rva): floats(reader.read(base + rva, 4), 0, 1)[0] for rva in (0x6303EC, 0x6303F0, 0x630420)
        },
    )
    if cursor:
    ...
    if (
        reader.read(base + 0x62D4D0, 8) != cursor_header
        or reader.read(base + 0x469A570, 16) != controller_header
    ):
