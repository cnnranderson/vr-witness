...
def sample(reader, base):
    ...
    result = {"allocated_slots": count, "controllers": [], "native_buttons": {},
              "controller_array": hex(array), "native_input_object": hex(keyboard)}
    if renderer:
    ...
    if count:
        ...
        for i in range(count):
            offset = i * 0x38
            entry = {"slot": i, "orientation_components": floats(raw, offset, 4),
                     "position_components": floats(raw, offset + 0x10, 3)}
            if i == 0:
                entry.update(startup_controller_flag=raw[offset + 0x1C],
                             pad_press_edge=raw[offset + 0x1D], pad_release_edge=raw[offset + 0x1E],
                             pad_pressed=raw[offset + 0x1F], pad_touched=raw[offset + 0x20],
    ...
    if keyboard:
        ...
        for key, label in KEYS.items():
            ...
            result["native_buttons"][label] = {"key_code": hex(key), "raw_bits": bits,
