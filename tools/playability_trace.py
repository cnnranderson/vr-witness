...
def sample(reader: ProcessReader, base: int) -> dict:
    ...
    result = dict(
        head=dict(
            orientation_components=floats(head, 0, 4),
            position_components=floats(head, 0x10, 3),
            valid_byte=head[0x1C],
        ),
        menu=dict(
            fade_by_rva_61e994=floats(reader.read(base + 0x61E994, 4), 0, 1)[0],
        ...
        cursor_scalars_by_rva=dict(
            zip(
                (hex(rva) for rva in range(0x62FF14, 0x62FF40, 4)),
        ...
        render_targets_by_rva={
            hex(rva): hex(struct.unpack("<Q", reader.read(base + rva, 8))[0])
            for rva in (0x469A5D0, 0x469A5D8, 0x469AB58, 0x469AB70, 0x630C78, 0x630BD0, 0x630BD8)
        },
    ...
    if renderer:
        ...
        result["renderer"] = dict(
            ...
            target_pointers_by_offset={
                hex(offset): hex(struct.unpack_from("<Q", raw, offset)[0]) for offset in (0x38, 0x40)
            },
        )
    if cursor:
        raw = reader.read(cursor, 0xCC)
        result["cursor"] = dict(
            screen_coordinates=floats(raw, 0x18, 2),
            alternate_coordinates=floats(raw, 0x20, 2),
            flag_39=raw[0x39],
            flag_3a=raw[0x3A],
            projection_scalars=floats(raw, 0x94, 2),
            ray_origin=floats(raw, 0x9C, 3),
            plane_origin=floats(raw, 0xA8, 3),
            plane_u=floats(raw, 0xB4, 3),
            plane_v=floats(raw, 0xC0, 3),
        )
    ...
    if count:
        ...
        for index in range(count):
            ...
            if index == 0:
                entry.update(
                    startup_model_flag=raw[offset + 0x1C],
