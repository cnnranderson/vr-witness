...
def summarize(events):
    ...
    for role in range(2):
        ...
        info = {
            "role": "left" if role == 0 else "right",
            "valid_samples": len(valid),
            "devices": sorted({h["device"] for h in valid}),
            "pressed_bits": [],
            "touched_bits": [],
            "axes": [],
        }
        ...
        for axis in range(5):
            if valid:
                ranges = [
                    [min(h["axes"][axis][k] for h in valid), max(h["axes"][axis][k] for h in valid)]
                    for k in range(2)
                ]
                info["axes"].append(
                    {
                        "axis": axis,
                        "types": sorted({h["types"][axis] for h in valid}),
                        "xy_range": ranges,
                        "motion_observed": any(hi - lo > 0.05 for lo, hi in ranges),
                    }
                )
    ...
    for s in samples:
        ...
        key = (
            ...
            bool(s["fade"]),
            bool(s["focused"]),
            *((h["device"], h["valid"], h["pressed"]) for h in s["hands"]),
        )
        if key != previous:
            result["changes"].append(
                {
                    "seconds": round((s["tick"] - first) / 1000, 3),
                    "mode": s["mode"],
                    "paused": bool(s["fade"]),
            ...
            previous = key
    result["limits"] = (
        "Sampled state can miss brief edges. A physical label needs a labeled one-control-at-a-time interval; legacy bit names are not physical button identities."
    )
    return result
...
def main():
    ...
    result = summarize(
        [json.loads(l) for l in args.log.read_text(encoding="utf-8-sig").splitlines() if l.strip()]
    )
    if args.json:
        print(json.dumps(result, indent=2))
        return
    print(
        f"{result['samples']} samples / {result['seconds']} seconds; modes {result['modes']}; unfocused {result['unfocused_samples']}; paused {result['paused_samples']}"
    )
    for hand in result["hands"]:
        print(
            f"{hand['role']}: devices {hand['devices']}; valid {hand['valid_samples']}; pressed bits {hand['pressed_bits']}; touched bits {hand['touched_bits']}"
        )
        for a in hand["axes"]:
            if a["motion_observed"]:
                print(f"  axis {a['axis']}, types {a['types']}, x/y ranges {a['xy_range']}")
