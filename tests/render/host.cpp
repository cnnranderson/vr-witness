...
int wmain(int argc, wchar_t** argv) {
    ...
    while (WaitForSingleObject(stop.get(), 0) == WAIT_TIMEOUT && GetTickCount64() - start < 90000) {
        ...
        if (TestPaused() != paused)
            ++pause_leaks; // Simulation/input-like caller.
        ...
        for (int eye = 0; eye != 2; ++eye) {
            TestBeginEye(eye);
            if (WaitForSingleObject(skip.get(), 0) != WAIT_OBJECT_0)
                TestCursor(nullptr, nullptr, false);
            ...
            if (WaitForSingleObject(skip_menu.get(), 0) != WAIT_OBJECT_0)
                TestMenu(true, eye, false);
        }
        stage = 0;
    ...
    std::cout << "{\"before_cursor\":" << before_cursor.load() << ",\"after_cursor\":" << after_cursor.load()
              ...
              << ",\"after_menu\":" << after_menu.load() << ",\"paused_redraws\":" << paused_redraws
              << ",\"paused_skips\":" << paused_skips << ",\"pause_leaks\":" << pause_leaks
              << ",\"unpaused_skips\":" << unpaused_skips << "}\n";
