...
class CursorMemory(Memory):
    ...
    def read(self, address, size):
        if address == 0x62D4D0:
            ...
            return struct.pack(
                "<Q", self.cursor + (0x1000 if self.replace and self.cursor_queries > 1 else 0)
            )
        if address == 0x60000:
            self.cursor_reads += 1
            raw = bytearray(size)
            struct.pack_into("<2f", raw, 0x18, 0.3, float("nan"))
...
class Guards(unittest.TestCase):
    def test_cursor_turnover_rejected(self):
        with self.assertRaisesRegex(ValueError, "metadata changed"):
            sample(CursorMemory(replace=True), 0)

    def test_null_cursor_not_dereferenced(self):
        m = CursorMemory(cursor=0)
        self.assertNotIn("cursor", sample(m, 0))
        self.assertEqual(m.cursor_reads, 0)

    def test_invalid_floats_and_pose_retained(self):
        result = sample(CursorMemory(), 0)
        self.assertAlmostEqual(result["cursor"]["screen_coordinates"][0], 0.3)
